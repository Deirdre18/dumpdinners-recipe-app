...
- Users can search a recipe by Recipe Name, Username, Ingredients, Category or Calories. Search navigation is easy. I have just recently added 1 further input field to add_recipe.html, which is Calories and user input number.
...
- Unfortunately I did not get time to add a user dashboard, for example showing all recipes 'Liked' or recipes viewed. I had hoped possibly to be able to get this done but unfortunately I ran out of time. However, I added 'username' to display (along with views, likes, recipe category and calories for each recipe) at top of recipe cards, so users can see exactly what recipes they have added. I did this in place of a dashboard, as I did not have time to create a dashboard at this point. 
...
- Usernames or category names which are double barrel, cause issues with the recipe cards, as they push recipe image down a line. At the last minute, I tried to solve this but due to time factors, I simply removed any double barrell names in MongoDB and added a note(*) in add_recipe.htmlm and edit_recipe.html in username and category_name input fields to ask users to use only SINGLE names. 
...
I also tested features such as 'Search' function to ensure it worked correctly and my findings are that it does work successfully (on both large and small screens). Using can search using Recipe Name, Username, Category, Ingredients or Calories. These are the results of tests using Search - [RECIPE SEARCH](https://github.com/Deirdre18/dumpdinners-recipe-app/blob/master/Mock-ups/Mock-ups%20DumpDinners-Search%20button.pdf)
